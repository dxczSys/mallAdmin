    ...
    <div class="reports-wrapper">
        ...
        <el-card class="chat-box">
            <div class="filter-by-time">
                <span class="fix-option" :class="{'current-label': index == 1}" @click="switchTime(15, 1)">近15天</span>
                <span class="fix-option" :class="{'current-label': index == 2}" @click="switchTime(30, 2)">近30天</span>
                <span class="fix-option" :class="{'current-label': index == 3}" @click="switchTime(1, 3)" style="margin-right: 10px;">近1年</span>
...
export default {
    ...
    methods: {
        ...
        formmateYear() {
            let arr = [], monthArr = [], data = new Date(), i = 0
            ...
            if (num != 1) {
                ...
                this.http({
                    ...
                    if (res.data.code == 200) {
                        this.initChatBar(timeArr, res.data.data.salesNum, res.data.data.ordersNum)
                        this.initOrderChat(res.data.data.goodsName, res.data.data.goodsOrders)
                    }
                }) 
            }else if (num == 1) {
                ...
                this.http({
                    ...
                    if (res.data.code == 200) {
                        this.initChatBar(month, res.data.data.salesNum, res.data.data.ordersNum)
                        this.initOrderChat(res.data.data.goodsName, res.data.data.goodsOrders)
                    }
                })
            }
