  ...
  <div class="apply-ad-wrapper">
    ...
    <div class="step-box">
      ...
      >
        <el-step
          title="申请"
          description="商户自主填写相关资料并付款"
        ></el-step>
        <el-step title="审核中" description="商城管理员正在审核中..."></el-step>
        <el-step
          title="已上架"
          v-if="activityStatus == 2 || activityStatus == 1"
          description="广告位已上线"
        ></el-step>
        <el-step
          title="失败"
          v-if="activityStatus == 3"
          :description="detailData.advertRefuseInfo || '不通过'"
        ></el-step>
        <el-step
          title="强制下架"
    ...
    <div class="apply-form">
      ...
      >
        ...
        <el-form-item label="广告位介绍">
          <img
            v-if="applyForm.adType == '1'"
            class="ad-introduction"
            src="~@/assets/img/zhiding-ad.png"
          />
          <img
            v-if="applyForm.adType == '2'"
            class="ad-introduction"
            src="~@/assets/img/cuxiao-ad.png"
          />
          <span v-if="applyForm.adType == '1'" class="ad-introduction-mess"
            >*广告位介绍：该广告位位于易码商城小程序首页置顶位，如左图所示。首页是用户查看最多的地方，且该位置位于最醒目，用户操作最容易触及的地方，我们在此为广大商户设置了20个展示商户基本信息的广告位，快来加入吧！</span
          >
          <span v-if="applyForm.adType == '2'" class="ad-introduction-mess"
            >*广告位介绍：该广告位位于易码商城小程序底部，如左图所示。该位置是用户最容易触及的地方，我们在此为广大商户设置了30个展示促销商品信息的广告位，可以大大提升自己的知名度，快来加入吧！</span
          >
        </el-form-item>
        <el-form-item
          v-if="applyForm.adType == '2'"
        ...
        >
          <el-select
            v-model="applyForm.goodsId"
            v-el-select-loadmore="handleChangePage"
            placeholder="请选择本店促销商品"
          >
            <el-option
              v-for="(item, index) in goodsList"
              :key="index"
              :label="`商品编码:${item.goodNumber || '无'} -- ${
                item.goodTitle
              }`"
              :value="item.id"
            ></el-option>
        ...
        <el-form-item label="申请时长" prop="timeType" required>
          <el-radio-group v-model="applyForm.timeType" :disabled="type == '1'">
            <el-radio
              ...
              :key="index"
              :label="item.id"
              >{{ item.advertPrice }}元/{{ item.advertDay }}天</el-radio
            >
          </el-radio-group>
        </el-form-item>
        <el-form-item
          v-if="type == 1 && detailData.advertApprovalStatus != '2'"
          label="申请时间"
        >
          <span>{{
            _dateFormat("YYYY-mm-dd HH:MM", detailData.advertCreateTime)
          }}</span>
        </el-form-item>
        <el-form-item
          v-if="
            type != 0 &&
            (detailData.advertApprovalStatus == '2' ||
              detailData.advertApprovalStatus == '4')
          "
          label="上架时间"
        >
          <span>{{
            _dateFormat("YYYY-mm-dd HH:MM", detailData.advertApprovalTime)
          }}</span>
        </el-form-item>
        <el-form-item
          v-if="
        ...
        >
          <span>{{
            _dateFormat("YYYY-mm-dd HH:MM", detailData.advertExpireTime)
          }}</span>
        </el-form-item>
        <el-form-item
          v-if="
            type == 1 &&
            detailData.advertApprovalStatus == '2' &&
            getOverDay(detailData.advertExpireTime) < 3
          "
          label="*温馨提示"
        >
          <span style="color: #e6a23c"
            >您的广告位即将到期，为了不影响您的使用，请及时续费，否则到期之后会下线该广告</span
          >
        </el-form-item>
        <el-form-item
          v-if="type == 1 && detailData.advertApprovalStatus == '3'"
          label="拒绝原因"
        >
          <span>{{ detailData.advertRefuseInfo }}</span>
        </el-form-item>
        <el-form-item
          v-if="type == 1 && detailData.advertApprovalStatus == '4'"
          label="下架原因"
        >
        ...
        >
          <span
            >申请费用已退回至原账户，请注意查收。如有为题，请联系管理员。</span
          >
        </el-form-item>
        <el-form-item>
          <el-button
            v-if="type == 1"
            @click="$router.push({ name: 'user-ad-manager' })"
            >返回</el-button
          >
          <el-button
            type="primary"
            v-if="
              type == 1 &&
              detailData.advertIsExpire == '2' &&
              detailData.advertApprovalStatus == '2'
            "
            @click="(type = 2), (applyForm.timeType = '')"
            >续费</el-button
          >
          <el-button
            v-if="type != 1"
            ...
            >取消</el-button
          >
          <el-button v-if="type == 2" type="primary" @click="openPayType"
            >立即续费</el-button
          >
          <el-button v-if="type == 0" type="primary" @click="openPayType"
            >提交申请</el-button
          >
        </el-form-item>
      </el-form>
    ...
    >
      ...
      <div slot="footer">
        <el-button @click="(payType = '1'), (typeDialogVisible = false)"
          >取 消</el-button
        >
    ...
    >
      <div class="alipy-logo-box">
        <img
          v-if="payType == '1'"
          src="~@/assets/img/wechat_logo.png"
          alt="微信支付"
        />
      ...
      <div
        class="pay-box"
        :class="{ wechat: payType == '1', zhifubao: payType == '2' }"
      >
        <div class="alipy-content">
          <div class="alipy-content-left">
            ...
            <div v-if="payType == '1'" style="padding-top: 15px">
              打开手机微信扫描二维码支付
            </div>
            <div v-else style="padding-top: 15px">
              打开手机支付宝扫描二维码支付
            </div>
            <div style="color: #0ae; padding-top: 15px">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        goodsId: [
          { required: true, message: "请选择本地促销商品", trigger: "change" },
        ],
        timeType: [
