      ...
      <div class="filter-form">
        ...
        <div class="filter-form-row">
          <div class="filter-form-col">
            <label>销售状态</label>
            <el-select
              v-model="filterForm.sellState"
              placeholder="请选择销售状态"
            >
              <el-option
                v-for="(item, index) in sellStateList"
                :key="index"
                :label="item.label"
                :value="item.value"
              ></el-option>
    ...
    <div class="table-content-box">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{
              scope.row.goodIsUpperShelf == "1" ? "在售" : "已下架"
            }}</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <span>{{
              _dateFormat("YYYY-mm-dd HH:MM", scope.row.goodCreateTime)
            }}</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <div>
              <el-button @click="toEdit(scope.row)" type="text"
                >编辑|详情</el-button
              >
            </div>
            <div>
              <el-button
                v-if="scope.row.goodIsUpperShelf == '1'"
                @click="immediatelyShelf(scope.row)"
                type="text"
                style="color: #e6a23c"
                >立即下架</el-button
              >
            ...
            <div>
              <el-button
                v-if="scope.row.goodIsUpperShelf == '2'"
                @click="immediatelyPut(scope.row)"
                type="text"
                >立即上架</el-button
              >
            </div>
