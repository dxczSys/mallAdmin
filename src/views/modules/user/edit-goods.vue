    ...
    >
      ...
      <div class="base-mess-box">
        <el-form-item label="商品类目" prop="kindsId" required>
          <filter-tree
            :disabled="true"
            v-model="releaseForm.kindsId"
            :data="kindsTree"
            placeholder="类目搜索"
            width="400"
          ></filter-tree>
          <div class="wran-word">
            *提示：请选择商品类目，获取要发布商品的规格条件
          </div>
        </el-form-item>
        <el-form-item label="商品标题" prop="goodsTitle" required>
          <el-input
            v-model="releaseForm.goodsTitle"
            placeholder="最多允许输入60个汉字"
            style="width: 700px"
            maxlength="60"
            show-word-limit
          ></el-input>
          <div class="wran-word">
            *温馨提示：标题请勿使用敏感词，否则会被强制下架
          </div>
        </el-form-item>
        ...
        <el-form-item label="商品图片">
          <upload-file
            :filelist="releaseForm.assistUrls"
            :multiple="true"
            :limit="5"
          ></upload-file>
        </el-form-item>
      ...
      >
        ...
        <div style="margin-left: 20px" class="wran-word">
          *提示：销售信息是根据商品类目下的规格条件来组装数据，请依据必填项，填写所有信息，方可发布商品
        </div>
      </div>
      <div v-if="releaseForm.kindsId" class="base-mess-box">
        <el-form-item
          label="颜色分类"
          v-if="formatList.isColorPicker == 1"
          required
        >
          <div class="color-picker-custom">
            <div
              class="color-picker-custom-item"
              v-for="(item, index) in colorPickerTable"
              :key="index"
            >
              <color-picker
                width="200"
                v-model="item.nomalColor"
              ></color-picker>
              <el-input
                v-model="item.customColor"
                placeholder="自定义"
                style="width: 200px"
              ></el-input>
              <el-button
                @click="addOneColorTable"
                type="primary"
                :disabled="!item.nomalColor && !item.customColors"
                icon="el-icon-plus"
              ></el-button>
            </div>
          </div>
          <div class="wran-word">
            *温馨提示：请尽量选择标准色系，如果自定义颜色，请谨慎，以免客户难以辨识
          </div>
        </el-form-item>
        <el-form-item
          required
          v-for="(item, index) in formatList.formatArr"
          :key="index"
          :label="item.formatName"
        >
          <el-checkbox-group v-model="item.modelValue">
            <el-checkbox
              v-for="(i, j) in item.valueList"
              :key="j"
              :label="i.name"
            ></el-checkbox>
          </el-checkbox-group>
        ...
        <el-form-item label="商品销售规格" v-if="assemTableData.length">
          <el-table :data="assemTableData" style="width: 100%" border>
            <el-table-column
              v-for="(item, index) in coloumNameArr"
              :key="index"
              :prop="item.value"
              :label="item.label"
              width="180"
        ...
        <el-row :gutter="10">
          <el-col :span="12">
            <el-form-item label="一口价" prop="onePrice" required>
              <money-input
                v-model="releaseForm.onePrice"
                style="width: 360px"
              ></money-input>
              <span>元</span>
              <div class="wran-word">
                *温馨提示：一口价一般是其中一件商品的最低价，博取客户的眼球
              </div>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="总数量" prop="total" required>
              <el-input
                :disabled="assemTableData.length != 0"
                type="number"
                v-model="releaseForm.total"
                style="width: 360px"
              ></el-input>
              <span>件</span>
            </el-form-item>
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="12">
            <el-form-item label="商品编码">
              <el-input
                v-model="releaseForm.barCode"
                style="width: 360px"
                placeholder="自主商品编码(如：YM355210012)"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item label="特别说明">
          <el-input
            type="textarea"
            v-model="releaseForm.specialDescription"
            rows="4"
            maxlength="200"
            show-word-limit
            placeholder="商品特别说明，例如：尺码非标准尺码"
          ></el-input>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        mainUrl: [
          { required: true, message: "请填写上传商品主图", trigger: "blur" },
        ],
        onePrice: [
          { required: true, message: "请填写商品一口价", trigger: "blur" },
        ],
        total: [{ required: true, message: "请填写总数量", trigger: "blur" }],
        postageMore: [
          { required: true, message: "请填写邮费规则", trigger: "blur" },
        ],
        postage: [
          { required: true, message: "请填写邮费规则", trigger: "blur" },
        ],
  ...
  computed: {
    ...mapState('mall', ['mall_id', 'shop_id', 'floor_id'])
  },
  methods: {
    addOneColorTable() {
    ...
    checkData() {
      if (
        this.formatList.isColorPicker == 1 ||
        this.formatList.formatArr.length
      ) {
        if (!this.assemTableData.length) {
          ...
          for (let i = 0; i < this.assemTableData.length; i++) {
            if (
              !this.assemTableData[i].price ||
              !this.assemTableData[i].amount
            ) {
    ...
    assembleGoodsData() {
      let arr = [],
        ids = this.releaseForm.kindsId.split(","),
        self = this;
      if (this.assemTableData.length) {
        ...
        arr.push({
          ...
          goodClassTwo: ids[1],
          goodClassThree: ids[2],
          goodIdentificationCode: this.assemTableDataCopy[0]
            .goodIdentificationCode,
                ...
                this.http(
                  {
                    ...
                    data: {
                      ...
                      goodShopMall: this.mall_id,
                      goodShop: this.shop_id,
                      goodShopFloor: this.floor_id,
                      ...
                      tGoodInfo: {
                        ...
                        goodSpecialDescription: this.releaseForm
                          .specialDescription,
                      },
      ...
      this.http(
        ...
        (res) => {
          if (res.data.code == 200) {
            ...
            this.releaseForm.specialDescription =
              obj.tGoodInfo.goodSpecialDescription || "";
            this.colorPickerTable = obj.bigGood.colorPickerTable;
            this.assemTableData = obj.bigGood.assemTableData;
            this.assemTableDataCopy = JSON.parse(
              JSON.stringify(obj.bigGood.assemTableData)
            );
