...
export default {
  ...
  computed: {
    ...mapState('mall', ['shop', 'shop_id'])
  },
  mounted() {
    this.user = {
      id: this.shop.id,
      avatar: window.SITE_CONFIG["fileUrl"] + this.shop.shopSign,
      displayName: this.shop.abbreviation,
    }
    this.getConcatList()
    // this.initEmoji()
    this.receiveMessage()
  ...
  methods: {
    ...
    getConcatList() {
      this.http(
        {
          url: `merchant/vxchat/getChatListByShopId?sId=${this.shop_id}`,
          method: "get",
        },
        (res) => {
          if (res.data.code === 200) {
