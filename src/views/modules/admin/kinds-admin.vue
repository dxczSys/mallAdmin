    ...
    <div class="kinds-admin-box">
      ...
      <div v-if="currentLevel > 2" class="kinds-right">
        ...
        >
          ...
          <div>
            <el-button @click="handleNewCondition" type="primary" size="mini"
              >新增规格</el-button
            >
          ...
          >
            ...
            <el-form-item label="规格条件" required>
              <el-tag
                v-for="(value, j) in item.conditionArr1"
                :key="j"
                closable
                @close="deleteItem(index, j, value)"
                style="margin-right: 10px; color: #409eff"
                >{{ value.name }}</el-tag
              >
              <span
                v-if="!item.conditionArr.length"
              ...
              ></el-input>
              <span
                class="add-floor-button"
                v-if="!item.isAddItem"
                @click="openAdd(item, index)"
              >
            ...
            <span
              ...
              class="delete-condition-item el-icon-delete"
            ></span>
          </div>
          <div style="padding-left: 20px; text-align: right; margin-top: 20px">
            <el-button @click="handleSaveCondition" type="primary" size="small"
              >保存</el-button
            >
          </div>
        </el-form>
      ...
      <div v-if="currentLevel === 1" class="kinds-right">
        ...
        <div class="kinds-right-header" slot="header">
          ...
          <el-button type="primary" size="small" @click="saveSelectKinds"
            >保存</el-button
          >
        </div>
        <div class="kinds-list">
          ...
          <el-checkbox-group
            v-model="kindsCheckbox"
            class="kinds-list-checkbox"
          >
            <el-checkbox
              v-for="(item, index) in kindsList"
              :key="index"
              :label="item.id"
              :disabled="!!item.shopMallName"
            >
...
export default {
  ...
  methods: {
    expandTree(data, node, el) {
      document.getElementById(`new${data.id}`) &&
        (document.getElementById(`new${data.id}`).style.display =
          "inline-block");
    },
    collapseTree(data, node, el) {
      document.getElementById(`new${data.id}`) &&
        (document.getElementById(`new${data.id}`).style.display = "none");
    },
    renderTree(h, { node, data, store }) {
      if (node.level === 1) {
        return (
          <div class="nomal-level">
            <span class="nomal-level-label" id={`span${data.id}`}>
              {data.label}
            </span>
            <input
              class="edit-input"
    ...
    addChild(node, data) {
      ...
      this.$refs.myTree.append(
        { id: tempId, label: "", isNew: 1, children: [] },
        data.id
      );
      this.$nextTick((_) => {
